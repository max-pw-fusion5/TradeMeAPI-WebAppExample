...
namespace TradeMe_Test_WebApp_001.Controllers
{
    ...
    public class OAuthController : ControllerBase
    {
        ...
        public IActionResult GetRequestToken()
        {
            ...
            OAuthHelper.setSecret(OAuthHelper.RequestTokenKeyName, matches.Groups[1].ToString());
            OAuthHelper.setSecret(OAuthHelper.RequestTokenSecretKeyName, matches.Groups[2].ToString());

            //TODO: Generate (and send/redirect to/open browser with?) URL for user to log in and give permission to application 

            string authorizeUrl = string.Format("https://secure.tmsandbox.co.nz/Oauth/Authorize?oauth_token={0}", OAuthHelper.getSecret(OAuthHelper.RequestTokenKeyName));
        ...
        public IActionResult Callback(string oauth_token, string oauth_verifier)
        {
            ...
            if (string.IsNullOrWhiteSpace(oauth_token) || string.IsNullOrWhiteSpace(oauth_verifier) || 
                OAuthHelper.getSecret(OAuthHelper.RequestTokenKeyName) == null || 
                OAuthHelper.getSecret(OAuthHelper.RequestTokenSecretKeyName) == null)
            {
            ...
            OAuthHelper.getSecret(OAuthHelper.RequestTokenSecretKeyName), 
            oauth_verifier, 
            OAuthHelper.getSecret(OAuthHelper.RequestTokenKeyName));
            ...
            OAuthHelper.setSecret(OAuthHelper.AccessTokenKeyName, matches.Groups[1].ToString());
            OAuthHelper.setSecret(OAuthHelper.AccessTokenSecretKeyName, matches.Groups[2].ToString());
